...
private:
    ...
    // Native window handle.  HWND on Windows (via pt_window_native_win32
    ...
    // Scratch buffer reused across frames for the RGBA32F -> BGRA8 pack
    // step in PresentOutput().  Allocating a fresh vector every frame at
    // 1080p / 4K was several MB of heap churn on top of the CPU tracer;
    // this stays sized to the current swapchain and only re-allocates on
    // resize.  Passed directly to SetDIBitsToDevice.
